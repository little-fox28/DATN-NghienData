...
class DataLoader:
    ...
    def load_to_staging_and_transform(self, csv_file_path: Path, staging_table: str = 'stg_loan', sp_name: str = 'sp_load_star_schema') -> bool:
        ...
        """
        try:
            df_clean = self.load_csv(csv_file_path)
            ...
            with self.engine.begin() as conn:
                self.truncate_table(conn, staging_table)
                self.bulk_insert(conn, df_clean, staging_table)
                self.run_stored_procedure(conn, sp_name)
