...
class EnrichPayload(BaseModel):
    ...
    ml_risk_tier: Optional[str] = Field(None, description="Phân hạng rủi ro")
    top_positive_factors: Optional[list] = Field(None, description="Top yếu tố tích cực")
    top_negative_factors: Optional[list] = Field(None, description="Top yếu tố tiêu cực")
    recommended_interest_rate: Optional[float] = Field(None)
    max_credit_limit: Optional[float] = Field(None)
    monthly_payment_estimate: Optional[float] = Field(None)
    total_interest_estimate: Optional[float] = Field(None)
