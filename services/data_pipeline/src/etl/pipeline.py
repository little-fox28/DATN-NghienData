...
=======
    def load_predictions(self) -> bool:
        ...
        Data flow:
            predictions.csv → stg_predictions → sp_load_ml_predictions → FactMLPrediction

        Yêu cầu predictions.csv must be generated by export_predictions.py before calling this function..
        """
        logger.info("=" * 60)
        logger.info("Starting ML Predictions Load Phase")
        logger.info("=" * 60)

        if not self.predictions_path or not self.predictions_path.exists():
            logger.error(f"Load predictions failed - file not found: {self.predictions_path}")
            logger.error("Please run export_predictions.py first to generate the predictions.csv file.")
            return False
